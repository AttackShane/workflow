...
#### 5. 小地图（`modules/workflow-minimap.js`）

- **功能**: 画布小地图，提供全局视图导航
- **特性**:
  - 节点缩略图渲染（按比例缩小到小地图尺寸）
  - 视口指示器（蓝色矩形显示当前可视区域）
  - 交互式导航（点击/拖拽视口指示器移动画布视口）
  - 性能优化（视口剔除、防抖渲染）

#### 6. 网格吸附（`modules/workflow-align.js` 增强）

- **功能**: 节点拖拽自动吸附到网格点
- **特性**:
  - 画布背景网格渲染（CSS Grid 背景）
  - 节点移动时自动吸附到最近网格点
  - 自定义网格大小（默认 20px）
  - 吸附阈值可配置

#### 7. 节点锁定（`modules/workflow-selection.js` 增强）
...
#### 8. 批量操作增强（`modules/workflow-manager.js` 增强）

- **功能**: 管理页面批量操作
- **特性**:
  - 多选模式（复选框 + 全选/取消全选）
  - 批量删除工作流（含确认对话框）
  - 拖拽排序工作流列表（HTML5 Drag & Drop API）
  - 多选时点击不跳转页面

#### 9. 版本对比（`modules/workflow-version.js`）
...
#### 10. 导出 SVG（`modules/workflow-svg-export.js`）
...
- `workflow-align.js` `distribute` 方法重复保存历史：修复为仅在操作完成时保存一次
...
- GET `/utils/storage.js` 404：修复 `workflow-keyboard.js` 导入路径
