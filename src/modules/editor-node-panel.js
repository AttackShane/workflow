...
export class WorkflowNodePanel {
    ...
    static LOGIC_OPTIONS_HTML = WorkflowNodePanel.LOGIC_OPTIONS.map(
        (o) => '<option value=' + o.value + '>' + o.label + '</option>'
    ).join('');

    static NEW_BRANCH_ITEM_HTML =
    ...
    _wfAddCondItem(btn) {
        ...
        item.dataset.condIndex = String(condIndex);
        item.innerHTML =
            '<div class="cond-item-header">' +
            '<span class="cond-item-title">条件 ' +
            (condIndex + 1) +
            '</span>' +
            ...
            '" data-cond-index="' +
            condIndex +
            '" data-side="right" title="选择引用">🔗</button>' +
            '</div></div>';
        list.appendChild(item);
    ...
    _updateCondLogicVisibility(condList) {
        ...
        if (count > 1) {
            if (!condLogic) {
                ...
                condLogic.innerHTML =
                    '<label class="cond-label">逻辑</label><select class="property-input cond-logic-select">' +
                    WorkflowNodePanel.LOGIC_OPTIONS_HTML +
                    '</select>';
    ...
    _conditionValueToText(valueObj) {
    ...
    _renderConditionItem(cond, i, branchIndex, nodeId) {
        const leftText = this._conditionValueToText(cond.left);
        const rightText = this._conditionValueToText(cond.right);
    ...
    renderPropertyPanel(targetNode) {
        ...
        if (targetNode.outputParams === undefined) {
            ...
            if (targetNode.parameters?.node_outputs && typeof targetNode.parameters.node_outputs === 'object') {
                targetNode.outputParams = this.node.paramEditor._paramsFromNodeOutputs(
                    targetNode.parameters.node_outputs
                );
            }
        ...
        if (targetNode.inputParams === undefined) {
            ...
            if (targetNode.parameters?.node_inputs && typeof targetNode.parameters.node_inputs === 'object') {
                targetNode.inputParams = this.node.paramEditor._paramsFromNodeOutputs(
        ...
        params.forEach((param) => {
            ...
            const hint = param.description
                ? `<div class="hint">${StringUtils.escapeHtml(param.description)}</div>`
                : '';
            ...
            if (param.name === 'branches' && targetNode.type === 'condition') {
                ...
                branches.forEach((branch, i) => {
                    const name =
                        branch && branch.name ? branch.name : i === 0 ? 'True' : i === 1 ? 'False' : `Branch ${i}`;
                    ...
                    const logicOptions = WorkflowNodePanel.LOGIC_OPTIONS.map((o) => {
                        const sel = o.value === logic ? ' selected' : '';
                        return `<option value="${o.value}"${sel}>${o.label}</option>`;
                    }).join('');
                    const condsHtml = conds.map((c, ci) => this._renderConditionItem(c, ci, i, targetNode.id)).join('');
                    const condLogicHtml =
                        conds.length > 1
                            ? `
                ...
                switch (param.type) {
                    ...
                    case 'textarea':
                        ...
                        break;
                    case 'select':
                        const selectOptions = (param.options || []).map((opt) => {
        ...
        detailContainer.innerHTML = `
            <div class="property-panel-section">
                ...
                ${this.node.paramEditor.renderMergeGroups(targetNode)}

                ${this.node.paramEditor.renderLoopVariables(targetNode)}

                ${this.node.paramEditor.renderLoopIntermediateVariables(targetNode)}
                ...
                <div id="inputParamsList">
                    ${this.node.paramEditor.renderInputOutputParams(targetNode.inputParams || [], 'input', targetNode.id)}
                </div>
                ...
                <div id="outputParamsList">
                    ${this.node.paramEditor.renderInputOutputParams(targetNode.outputParams || [], 'output', targetNode.id)}
                </div>
    ...
    renderBatchEditPanel(selectedNodes) {
        const detailContainer = document.getElementById('nodeDetail');
        if (!detailContainer) return;
        ...
        uniqueTypes.forEach((type) => {
            ...
            const params = info.parameters || [];
            params.forEach((p) => {
                if (p.name === 'branches' || p.name === 'options' || p.name === 'categories' || p.name === 'variables')
                    return;
                const key = p.name;
        ...
        if (sharedParams.length === 0) {
            paramsHtml =
                '<p style="color:var(--text-secondary);padding:1rem;text-align:center;">所选节点没有公共参数可批量修改</p>';
        } else {
            paramsHtml = sharedParams
                .map((param) => {
        ...
        if (btnApply) {
            btnApply.addEventListener('click', () => {
                ...
                this.node.core.saveHistory('messages.batchEditParams');
                this.node.ui.showMessage(`已批量修改 ${nodeCount} 个节点的参数`, 'success');
                this.node.ui.showSummaryPanel();
            });
        }
    ...
    saveNodeDetail(nodeId, silent) {
        ...
        const descEl = /** @type {HTMLInputElement|null} */ (document.getElementById('prop_nodeDescription'));
        ...
        params.forEach((param) => {
            ...
            if (
                (param.name === 'branches' && targetNode.type === 'condition') ||
            ...
            const input = /** @type {HTMLInputElement|null} */ (document.getElementById(`prop_${param.name}`));
            if (input) {
                let value;
        ...
        this.node.paramEditor.saveMergeGroupVars(targetNode);
        this.node.paramEditor.saveLoopVariables(targetNode);
        this.node.paramEditor.saveLoopIntermediateVariables(targetNode);

        this.node.render._reRenderNode(nodeId);
        this.node.ui.updateEdges();
        this.node.core.saveHistory('actions.editNode');
        if (!silent) {
            this.node.ui.showMessage(t('actions.nodeSaved'), 'success');
        }
    }
    _autoSaveTimer = null;
    _autoSaveNodeId = null;

    _setupAutoSave(nodeId) {
    ...
    _handleAction(e) {
        ...
        const activeNodeId = nodeId || this.node.core.selectedNode;

        switch (action) {
            case 'addInputParam':
                this.node.paramEditor.addInputParam(nodeId);
                this._scheduleAutoSave(nodeId);
                break;
            case 'addOutputParam':
                this.node.paramEditor.addOutputParam(nodeId);
                this._scheduleAutoSave(nodeId);
                break;
            case 'wfAddCondItem':
