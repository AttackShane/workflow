...
export class VirtualScroll {
    constructor(options) {
        ...
        // DOM 元素不触发 resize 事件，用 ResizeObserver 监听容器尺寸变化
        this._resizeObserver =
            typeof ResizeObserver !== 'undefined' ? new ResizeObserver(this.boundResizeHandler) : null;
    ...
    setContent(text, originalLineCount = null) {
        ...
        this._originalLineCount = lineCount;
    ...
    updateFontSize() {
        ...
        // 使用原始行数（高亮 HTML 行数可能与原文不一致）
        const lineCount = this._originalLineCount ?? this.lines.length;
        const totalHeight = lineCount * this.lineHeight;
        this.buffer.innerHTML = `<div style="height:${totalHeight}px;"></div>`;
        this.renderLineNumbers(lineCount);
