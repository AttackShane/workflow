...
export const nodeHandlers = {
    ...
    llm: (data, params, ctx) => {
        data.inputs = {
            ...
            settingOnError: params.settingOnError || {
                switch: false,
                processType: 1,
                timeoutMs: 600000,
                retryTimes: 0,
    ...
    plugin: (data, params, ctx) => {
        const apiParam = (params.apiParam || ctx.node.apiParam || []).map((ap) => {
            if (ap.input) {
    ...
    intent: (data, params, ctx) => {
        data.inputs = {
            chatHistorySetting: params.chatHistorySetting ||
                ctx.node.chatHistorySetting || { enableChatHistory: false, chatHistoryRound: 3 },
            inputParameters: ctx.inputParams,
    ...
    question: (data, params, ctx) => {
        ...
        if (params.llmParam) {
            if (Array.isArray(params.llmParam)) {
                ...
                llmParam.systemPrompt =
                    params.llmParam.find((p) => p.name === 'systemPrompt')?.input?.value?.content || '';
                ...
                Object.entries(params.llmParam).forEach(([key, value]) => {
                    if (key === 'systemPrompt') {
                        ...
                        const valueType =
                            typeof value === 'number' ? (value % 1 === 0 ? 'integer' : 'float') : 'string';
                        llmParam[String(index++)] = {
                            name: key,
                            input: {
                                type: valueType,
        ...
        const dynamicOption = params.dynamic_option || { type: 'string', value: { type: 'literal', content: '' } };
