...
export function processContainerNode(data, params, ctx, type) {
    ...
    if (type === 'loop') {
        ...
        if (params.iterationVariable !== undefined)
            data.inputs.iterationVariable = { type: 'literal', content: params.iterationVariable };
        ...
        if (params.batchSize !== undefined)
            data.inputs.batchSize = params.batchSize || { type: 'integer', value: { type: 'literal', content: 100 } };
        if (params.concurrentSize !== undefined)
            data.inputs.concurrentSize = params.concurrentSize || {
                type: 'integer',
    ...
    if (outputs) {
        for (const [on, od] of Object.entries(outputs)) {
            ...
            const ot = outputType === 'list' ? 'list' : outputType || 'string';
            const innerType = outputType === 'list' ? itemsType || 'string' : outputType || 'string';
